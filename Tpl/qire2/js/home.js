...
var FF = {
	...
	'Comment': {
	 'Default': function($ajaxurl) {
		...
		'Post': function(){
			...
			var $data = "cm_sid="+Sid+"&cm_cid="+Id+"&cm_content="+$("#comment_content").val()+"&cm_user="+$("#cm_user").val();
			$.ajax({
				type: 'post',
			...
			$(".Gold>span").mouseout(function(){
				...
				for(i=0;i<5;i++){
					if(i<Math.round($score)){
						if(i == parseInt($score)){
							$(".Gold>#"+i+"").css({background:$bgurl+" 25px 0 repeat"});
						}else{
							$(".Gold>#"+i+"").css({background:$bgurl});
						}
					}else{
	...
	'History': {
		...
		'Create': function($id){
			...
			html +='<dt><span href="javascript:void(0)" onclick="FF.History.Clear();">清空</span> | <span href="javascript:void(0)" onclick="FF.History.Hide();">关闭</span></dt>';
