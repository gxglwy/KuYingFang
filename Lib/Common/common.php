...
* 这是自己添加的函数
* 获取系统tag列表
* 参数: cid:1  (表示查询ID为1的栏目下的所有影片包含的标签信息，包括子栏目下影片,不填参数查询所有)
*/
function ff_mysql_tag($tag){

	$tag = ff_param_lable($tag);
	//$limit = !empty($tag['limit']) ? $tag['limit'] : NULL;
	//$limit = !empty($tag['cid']) ? $tag['cid'] : NULL;
	
	//优先从缓存调用
	if(C('data_cache_tagslist')){
			$data_cache_name = md5(C('data_cache_tagslist').implode(',',$tag));
			$data_cache_content = S($data_cache_name);
			if($data_cache_content){
					return $data_cache_content;
			}
	...
	$rs = M('Tag');
	$sql = "select DISTINCT(g.tag_name) from ff_tag g left join ff_vod d on (g.tag_id = d.vod_id) left join ff_list l on (d.vod_cid = l.list_id or d.vod_cid = l.list_pid) ";
	
	if ($tag['cid']) {
			$sql.="where l.list_pid = ". $tag['cid']." or l.list_id = ".$tag['cid'];
	}
	$list = $rs->query($sql);
	
	//是否写入数据缓存
	if(C('data_cache_tagslist')){
			S($data_cache_name,$list,intval(C('data_cache_tagslist')));
	}
	return $list;
}
